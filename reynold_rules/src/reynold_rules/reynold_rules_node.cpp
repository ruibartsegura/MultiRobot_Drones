...
ReynoldRulesNode::nav_2_point_rule()
{
    ...
    if (--rendezvous_counter_ <= 0) {
      ...
      RCLCPP_INFO(get_logger(), "navigate: (%.2f, %.2f) -> (%.2f, %.2f)   (%.2f, %.2f, %.2f)",
                  pos.x, pos.y,
                  path.front().x, path.front().y, vec.x, vec.y, vec.z);
		}
	}
...
ReynoldRulesNode::rendezvous_protocol()
{
    ...
    const auto& position = robots_[i]->pose.pose.position;
		...
		for (size_t j = 0; j < robots_.size(); j++) {
			...
			// x += a_ij * d
      const auto STEP_SIZE = 1.0;
			x = add(x, mul(STEP_SIZE * a_ij, d));
		}
    ...
    auto wp = path.front();
    wp.z = position.z;
    if (i == 0) RCLCPP_INFO(get_logger(), "s=%ld, d=%.2f goal:(%.2f, %.2f)  (%.2f, %.2f) (%.2f, %.2f)",
                            path.size(), get_distance(position, wp), x.x, x.y,
                            position.x, position.y, wp.x, wp.y);
